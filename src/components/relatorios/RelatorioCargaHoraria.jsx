...
export default function RelatorioCargaHoraria({ execucoes, usuarios }) {
    ...
    const dataAgrupada = useMemo(() => {
        ...
        const grouped = execucoes.reduce((acc, exec) => {
            ...
            acc[groupKey].horasTrabalhadas += exec.tempo_total || 0;
            return acc;
        }, {});
            ...
            <Table>
                ...
                <TableBody>
                    {dataAgrupada.map((item, index) => {
                        const saldo = item.horasTrabalhadas - metaDiaria;
                        ...
                        if (item.horasTrabalhadas.toFixed(1) == metaDiaria.toFixed(1)) {
                           saldoFormatado = 'OK';
                        }
                        ...
                        return (
                            <TableRow key={index}>
                                ...
                                <TableCell className="text-center font-bold text-lg text-blue-600">
                                    {item.horasTrabalhadas.toFixed(1)}h
                                </TableCell>
